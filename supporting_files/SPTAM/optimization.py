...
class LocalBA(object):
    ...
    def set_data(self, adjust_keyframes, fixed_keyframes,w = 10):
        self.clear()
        for kf in adjust_keyframes:
            ...
            for m in kf.measurements():
                ...
                if pt not in self.mappoints:
                    # Cannot add the point w times but edge can be added w times between 2 nodes
                    # my changes

######################################################################## CHANGES ###########################################################################
                    f = open('log.txt', "a+")
                    #print("#######################",m.source == Measurement.Source.SEMANTIC)
                    if m.source == Measurement.Source.SEMANTIC:
                        self.optimizer.add_point(pt.id,pt.position,fixed = True)
                        f.write("--------------------------SEMANTIC POINT ADDED --------------------------------")
                        #print("--------------------------SEMANTIC POINT ADDED --------------------------------")
                        edge_id = len(self.measurements)
                        for j in range(edge_id,edge_id+w):
                            self.optimizer.add_edge(j,pt.id,kf.id,m)
                            #print("--------------------------SEMANTIC EDGE ADDED --------------------------------")
                            self.measurements.append(m)
                    else:
                        self.optimizer.add_point(pt.id, pt.position,fixed = False)
                        #print("--------------------------SPTAM POINT and EDGE ADDED --------------------------------")
                        edge_id = len(self.measurements)
        ...
        for kf in fixed_keyframes:
            ...
            for m in kf.measurements():
                if m.mappoint in self.mappoints:#my changes
                    edge_id = len(self.measurements)
                    if m.source == Measurement.Source.SEMANTIC:
                        for j in range(edge_id,edge_id+w):
                            self.optimizer.add_edge(j,m.mappoint.id,kf.id,m)
                            #print("--------------------------SEMANTIC EDGE ADDED --------------------------------")
                            self.measurements.append(m)
                    else:
                        self.optimizer.add_edge(edge_id, m.mappoint.id, kf.id, m)
                        #print("--------------------------SPTAM EDGE ADDED --------------------------------")
                        self.measurements.append(m)
