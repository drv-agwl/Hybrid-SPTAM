...
class KITTIOdometry(object):   # without lidar
    ...
    def __init__(self, path):
        ...
        self.left_images = self.listdir(os.path.join(path, 'image_0'))
        self.right_images  = self.listdir(os.path.join(path, 'image_1'))

        # Don't understand what these cam___ variables are, why are there multiple cam??
        ...
        cam00_02 = Cam(695.2416405707397, 692.6243332193645, 581.2702634975295, 364.903998390924, 1280, 720, 0.120008)
        cam03 = Cam(692.9653647090648, 690.940134937844, 577.6725250355717, 360.17678176013095, 1280, 720, 0.120008)
        # cam03 = Cam(721.5377, 721.5377, 609.5593, 172.854, 1241, 376, 0.53715)
        cam04_12 = Cam(707.0912, 707.0912, 601.8873, 183.1104, 1241, 376, 0.53715)

        path = os.path.expanduser(path)
        timestamps = np.loadtxt(os.path.join(path, 'times.txt'))
        self.left = ImageReader(self.listdir(os.path.join(path, 'image_0')),   # ImageReader returns a list of loaded images
            timestamps)
    ...
    def sort(self, xs):
        return sorted(xs, key=lambda x:float(x[-10:-4]))
